...
class sql_db_operations:
    ...
    def create_populate_Customers(self):
        customers_excel_data = pd.read_excel("C:/Users/risha/OneDrive/Documents/CSYE7380/CSYE_7380_GenAI_Theory_Practical_Implementation/Project for genAI/Categories.xlsx")
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for index, row in customers_excel_data.iterrows():
                    print(f"row = {row}")
                    insert_query_sql = 'INSERT INTO categories (ID, FIRST_NAME, LAST_NAME, STREET_ADDRESS, CITY, STATE, ZIPCODE, EMAIL, ADD_TO_MAILING_LIST, PHONE_NUMBER, OTHER_NOTES) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'
                    cursor.execute(insert_query_sql,
                                   (int(row['ID']), str(row['First Name']), str(row['Last Name']), str(row['Street Address']), str(row['City']), row['State'], row['Zip code'], row['Email'], row['Add to Mailing List?'], row['Phone Number'], row['Other Notes']))
            self.connection.commit()
        except Exception as e:
            print(f'An exception has occurred: {e}')
