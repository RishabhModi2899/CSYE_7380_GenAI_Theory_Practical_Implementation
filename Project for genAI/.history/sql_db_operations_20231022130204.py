        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for index, row in orderItems_excel_data.iterrows():
                    print(f"row = {row}")
                    insert_query_sql = 'INSERT INTO Order_Items (ID, ORDER_ID, MENU_ITEMS_ID, QUANTITY) VALUES (%s, %s, %s, %s)'
                    cursor.execute(insert_query_sql,
                                   (int(row['ID']), int(row['Order ID']), int(row['Sales Unit ID']), str(row['Price'])))
            self.connection.commit()
        except Exception as e:
            print(f'An exception has occurred: {e}')
