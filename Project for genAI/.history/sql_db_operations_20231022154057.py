        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for index, row in ordersDecember_excel_data.iterrows():
                    print(f"row = {row}")
                    insert_query_sql = 'INSERT INTO Orders_Table (PRODUCT_TYPES, PRODUCT_NAME, SALES_UNIT, VALUE_OF_SALES_UNIT, NUMBER_OF_SALES_UNIT_SOLD, ACTUAL_NUMBER) VALUES (%s, %s, %s, %s, %s, %s)'
                    cursor.execute(insert_query_sql,
