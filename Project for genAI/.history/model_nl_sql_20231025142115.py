...
class SQLApp:
    ...
    def conversation_continue(self, messages, temperature=0):
        ...
        sql_response = response.choices[0].message['content']
         print(f"OpenAI Response: {sql_response}")  # Debug print
    ...
    def add_prompts_conversation(self, _):
        ...
        sql_response, db_results = self.conversation_continue(context)
        print(type(db_results))
        print(str(sql_response))
        print(db_results)
        
        panels.clear()  # Clear previous panels
        
        panels.append(pn.Row('User:', pn.pane.Markdown(prompt, width=600)))
        panels.append(pn.Row('Assistant:', pn.pane.Markdown(sql_response, width=600, styles={'background-color': '#F6F6F6'})))
