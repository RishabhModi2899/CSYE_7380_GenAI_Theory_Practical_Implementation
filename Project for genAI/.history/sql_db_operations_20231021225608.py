        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for index, row in customers_excel_data.iterrows():
                    print(f"row = {row}")
                    insert_query_sql = 'INSERT INTO categories (ID, FIRST_NAME, LAST_NAME, STREET_ADDRESS, CITY, STATE, ZIPCODE, EMAIL, ADD_TO_MAILING_LIST, PHONE_NUMBER, OTHER_NOTES) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, )'
                    cursor.execute(insert_query_sql,
                                   (int(row['ID']), str(row['Product Types'])))
            self.connection.commit()
        except Exception as e:
