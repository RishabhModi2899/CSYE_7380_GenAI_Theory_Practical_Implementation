        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for index, row in menuItems_excel_data.iterrows():
                    ...
                    cursor.execute(insert_query_sql,
                                   (int(row['ID']), int(row['Product ID']), int(row['Sales Unit ID']), (row['Street Address']), str(row['City']), row['State'], str(row['Zip Code']), str(row['Email']), str(row['Add to Mailing List?']), str(row['Phone Number']), str(row['Other Notes'])))
            self.connection.commit()
        except Exception as e:
            print(f'An exception has occurred: {e}')
