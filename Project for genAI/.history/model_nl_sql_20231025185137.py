...
class SQLApp:
    ...
    def add_prompts_conversation(self, _):
        ...
        #Get the response. 
        response = self.conversation_continue(context)#Extract the sql from the response.
    ...
    def run_sql_query(self, _):
        print(f"SQL Query: {self.last_sql_response}")
        db_results = None
        if self.last_sql_response:
            ...
            if isinstance(db_results, list) and db_results and isinstance(db_results[0], dict):
                ...
                panels.append(pn.Row('DB Results:', pn.pane.DataFrame(df, width=600, styles={'background-color': '#F6F6F6'})))
...
button_run_sql = pn.widgets.Button(name="Run SQL Query")
interactive_run_sql = pn.bind(app.run_sql_query, button_run_sql)
dashboard = pn.Column(
    client_prompt,
    pn.Row(button_conversation, button_run_sql),
    pn.panel(interactive_conversation, loading_indicator=True, height=300),
    pn.panel(interactive_run_sql, loading_indicator=True, height=300),
)
dashboard.servable()